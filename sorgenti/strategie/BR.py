...
from piattaforme.bitstamp.bitstampRequests import (
	buy, getBalance, getOrderStatus, sell
)
...
def gestore( valore_attuale ):
	...
	try:
		...
		if soldi and not closing:
			...
			if round(
				valore_attuale, BR_Fattore_Approssimazionoe
			) < ultimo_valore or primo_acquisto:
	...
	try:
		...
		if "dev" in sys.argv[ 1: ]:
			...
			gestoreRapporti.FileAppend(
				...
				dt_string + " Acquisto al prezzo di [" + str( valore_attuale ) +
				"] usando " + str( soldi ) + VALUTA_SOLDI + " -> " +
			...
			if balance:
				gestoreRapporti.JsonWrites(
					"log/buy_balance.json", "w+", balance
				)
				...
				fee = float(
					balance[ f"{COPPIA_DA_USARE_NOME}_fee" ]
				) if f"{COPPIA_DA_USARE_NOME}_fee" in balance else None
				...
				if ultimo_id:
					...
					gestoreRapporti.JsonWrites(
						"log/buy_getOrderStatus.json", "w+", status
					)
					status = status[ "status" ] if "status" in status else None

				if soldi and soldi_balance and soldi != soldi_balance:
					gestoreRapporti.FileAppend(
						TRADING_REPORT_FILENAME, dt_string + " Discrepanza!: " +
						str( soldi_balance - soldi )
					)

				if soldi_balance and (
					not ultimo_id or not status or
					( status and status.lower() == "finished" )
				):

					# order
					soldi_balance_feeded = soldi_balance - (
						soldi_balance * fee / 100
					)
					#result = json.loads(buy(round(valore_attuale,5),round(soldi_balance / valore_attuale,8)))
					result = json.loads( buy( soldi_balance ) )

					gestoreRapporti.JsonWrites(
						"log/buy_buy.json", "w+", result
					)
					print( ">>> Buy result:" )
					print( result )
					if "id" in result:
						ultimo_id_ordine(
							result[ "id" ] if "id" in result else None
						)
						prezzo_ordine = float(
							result[ "price" ]
						) if "price" in result else None

						gestoreRapporti.FileAppend(
							TRADING_REPORT_FILENAME, dt_string +
							" Acquisto al prezzo di [" + str( prezzo_ordine ) +
							"] usando " + str( soldi_balance_feeded ) +
							VALUTA_SOLDI + " -> " + str(
							round( soldi_balance_feeded / prezzo_ordine, 8 )
							) + " " + VALUTA_CRIPTO
						)
...
def vendo( cripto, valore_attuale ):
	...
	try:
		...
		if "dev" in sys.argv[ 1: ]:
			...
			gestoreRapporti.FileAppend(
				TRADING_REPORT_FILENAME,
				dt_string + " Vendita al prezzo di [" + str( valore_attuale ) +
				"] usando " + str( cripto ) + VALUTA_CRIPTO + " -> " +
			...
			if balance:
				gestoreRapporti.JsonWrites(
					"log/sell_balance.json", "w+", balance
				)
				...
				if ultimo_id:
					status = json.loads( getOrderStatus( ultimo_id ) )
					gestoreRapporti.JsonWrites(
						"log/sell_getOrderStatus.json", "w+", status
					)
					status = status[ "status" ] if "status" in status else None
				...
				if cripto and cripto_balance and cripto != cripto_balance:
					gestoreRapporti.FileAppend(
						TRADING_REPORT_FILENAME,
						dt_string + " Aggiunta cripto manualmente: " +
						str( cripto_balance - cripto )
					)

				if cripto_balance and (
					not ultimo_id or not status or
					( status and status.lower() == "finished" )
				):
					# order
					...
					if "id" in result:
						...
						soldi_feeded = soldi_converted - (
							soldi_converted * fee / 100
						)
						...
						gestoreRapporti.FileAppend(
							TRADING_REPORT_FILENAME,
							dt_string + " Vendita al prezzo di [" +
							str( prezzo_ordine ) + "] usando " +
