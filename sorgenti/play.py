	...
	try:
		...
		if soldi:
			...
			gestoreRapporti.FileAppend(
				TRADING_REPORT_FILENAME, dt_string+" Inizio con " + str(round(soldi, 2)) + " " + str(MONETA))
		if cripto:
			gestoreRapporti.FileAppend(TRADING_REPORT_FILENAME,
									   dt_string+" Inizio con " +
									   str(round(cripto, 3)) +
									   " " + str(CRIPTOMONETA))
		...
		if soldi:
			gestoreRapporti.FileAppend(
				TRADING_REPORT_FILENAME, dt_string+" Finisco con " + str(round(soldi, 2)) + " " + str(MONETA))
	...
	with open(f'{DATASET_CARTELLA_PERCORSO}/{DATASET_NOME_DA_USARE}.csv') as csvFile:
		...
		for riga in datiStatici:
			...
			if riga and riga[0]:
				...
				pre_time_difference_in_minutes = time_difference / \
					timedelta(minutes=1)
	...
	if 'token' in request.args and encrypt_string(request.args['token']) == API_TOKEN_HASH:
		if 'valore' in request.args:
			ultimo_valore = commercialista(
				"ultimo_valore", float(request.args['valore']))[0]
			return str(ultimo_valore), 200
	return '', 404
	...
	if 'token' in request.args and encrypt_string(request.args['token']) == API_TOKEN_HASH:
		if 'valore' in request.args:
			valore_acquisto = commercialista(
				"valore_acquisto", float(request.args['valore']))[1]
			return str(valore_acquisto), 200
	return '', 404
