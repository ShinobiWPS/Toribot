...
def avvio():
	try:
		...
		if soldi:
			print(
				"Inizio con " + str( round( soldi, 2 ) ) + " " + str( MONETA )
			)
			gestoreRapporti.FileAppend(
				TRADING_REPORT_FILENAME, dt_string + " Inizio con " +
				str( round( soldi, 2 ) ) + " " + str( MONETA )
			)
		if cripto:
			gestoreRapporti.FileAppend(
				TRADING_REPORT_FILENAME, dt_string + " Inizio con " +
				str( round( cripto, 3 ) ) + " " + str( CRIPTOMONETA )
			)
			print(
				"Inizio con " + str( round( cripto, 3 ) ) + " " +
				str( CRIPTOMONETA )
			)
		sys.stdout.flush()
		...
		if soldi:
			...
			print(
				"Finisco con " + str( round( soldi, 2 ) ) + " " + str( MONETA )
			)
		if cripto:
...
def dati_statici():
	global STOP
	with open(
		f'{DATASET_CARTELLA_PERCORSO}/{DATASET_NOME_DA_USARE}.csv'
	) as csvFile:
		...
		for riga in datiStatici:
			...
			if riga and riga[ 0 ]:
				...
				pre_time_difference_in_minutes = time_difference / timedelta(
					minutes=1
				)
				time_difference_in_minutes = timedelta(
					minutes=pre_time_difference_in_minutes
				)
				if time_difference_in_minutes >= frequency:
					lastReferenceTime = datetime.strptime(
						riga[ 1 ], FORMATO_DATA_ORA
					)
...
def dati_da_Bitstamp_websocket():
	...
	try:
		...
		if cripto_balance != cripto:
			gestoreRapporti.FileAppend(
				TRADING_REPORT_FILENAME, dt_string + " Sync balance: " +
				str( round( cripto_balance - cripto, 8 ) ) + " " +
				str( CRIPTOMONETA )
			)
...
def ping():
	if 'token' in request.args and encrypt_string(
		request.args[ 'token' ]
	) == API_TOKEN_HASH:
		return 'online', 200
	return '', 404
...
def imposta_valore_acquisto():
	if 'token' in request.args and encrypt_string(
		...
		if 'valore' in request.args:
			valore_acquisto = commercialista(
				"valore_acquisto", float( request.args[ 'valore' ] )
			)[ 1 ]
			return str( valore_acquisto ), 200
	return '', 404
...
def forza_bilancio():
	if 'token' in request.args and encrypt_string(
		...
		return str(
			str( soldi_balance ) + " " +
			MONETA if soldi_balance else str( cripto_balance ) + " " +
			CRIPTOMONETA
		), 200
	return '', 404
...
def bilancio():
	if 'token' in request.args and encrypt_string(
		...
		return str(
			str( soldi ) + " " + MONETA if soldi else str( cripto ) + " " +
			CRIPTOMONETA
		), 200
	return '', 404
...
def bilancio_stimato():
	if 'token' in request.args and encrypt_string(
		...
		return str(
			str( soldi ) + " " +
			MONETA if soldi else str( cripto * ultimo_valore ) + " " +
			CRIPTOMONETA
		), 200
	return '', 404
...
def imposta_bilancio():
	if 'token' in request.args and encrypt_string(
		...
		if 'soldi' in request.args:
			cripto, soldi = portafoglio(
				"soldi", float( request.args[ 'soldi' ] )
			)
			return str( soldi ) + MONETA, 200
		if 'cripto' in request.args:
			cripto, soldi = portafoglio(
				"cripto", float( request.args[ 'cripto' ] )
			)
			return str( cripto ) + CRIPTOMONETA, 200
	return '', 404
