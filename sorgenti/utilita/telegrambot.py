...
class TelegramBot(object):
	...
	def messageHandler(self, message):
		...
		if content_type == 'text':
			if message['entities']:
				command = (message[content_type][int(message['entities'][0]['offset'])+1:int(message['entities'][0]['offset'])+int(message['entities'][0]['length'])])
				if command == "start":
				...
				if command == "status" or command == "stato":
					try:
						...
						if r and r.status_code == 200 and r.text:
							self.sendMessage(chat_id, str("Running" if r.text.lower() == "true" else "NOT running" ))
					except requests.exceptions.ConnectionError as ex:
						print(ex)
						self.sendMessage(chat_id, "Error, ToriBot API Error"+str(ex))
					except Exception as ex:
	...
	def getToribotAPI(self, endpoint):
		return requests.get(self.toribot_API_URL+endpoint+"?&token="+self.toribot_API_Token+"")
