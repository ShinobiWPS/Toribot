...
class TelegramBot( object ):
	...
	toribot_API_Token = ""
	toribot_API_URL = "http://" + toribot_API_Domain + ":" + str(
		toribot_API_Port
	) + "/"
		...
		if content_type == 'text':
			if message[ 'entities' ]:
				command = (
					...
					[ int( message[ 'entities' ][ 0 ][ 'offset' ] ) +
					1:int( message[ 'entities' ][ 0 ][ 'offset' ] ) +
				...
				if command == "balance" or command == "bilancio":
					try:
						...
						print( ex )
						self.sendMessage(
							chat_id, "Error, ToriBot API Error" + str( ex )
						)
					except Exception as ex:
						print( ex )
						self.sendMessage(
							chat_id, "Error, retrieving balance" + str( ex )
						)
					...
					try:
						...
						if r and r.status_code == 200 and r.text:
							self.sendMessage(
								chat_id,
								str(
								"Running" if r.text.lower() ==
								"true" else "NOT running"
								)
							)
					except requests.exceptions.ConnectionError as ex:
						print( ex )
						...
						print( ex )
						self.sendMessage(
							chat_id, "Error, retrieving status" + str( ex )
						)
				...
				if command == "stop_toribot":
					try:
						...
						print( ex )
						self.sendMessage(
							chat_id, "Error executing" + str( ex )
						)
	...
	def getToribotAPI( self, endpoint ):
		return requests.get(
			self.toribot_API_URL + endpoint + "?&token=" +
			self.toribot_API_Token + ""
		)
