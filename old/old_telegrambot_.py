...
def telegram( azione, text ):
	...
	def new_mex(
		msg
	):  # questa funzione viene eseuita se arriva un nuovo messaggio
		...
		if content_type == 'text':  # se il nuovo messaggio è un testo
			...
			# l'ultimo aromento, fà comparire la keyboard
			bot.sendMessage(
				chat_id, 'hai inviato un messaggio?', reply_markup=keyboard
			)
...
# ---------------------click su keyboard in chat---------------------------------
	...
	def new_query(
		msg
	):  # questa funzione viene eseuita se viene cliccato un tasto keyborad
